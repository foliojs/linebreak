...
describe 'unicode line break tests', ->
  ...
  skip = [812,   814,  848,  850,  864,  866,  900,  902,  956,  958, 1068, 1070, 1072, 1074, 1224, 1226,
  ...
  lines.forEach (line, i) ->
    return if not line or /^#/.test(line)
  
    [cols, comment] = line.split('#')
    codePoints = cols.split(/\s*[×÷]\s*/).slice(1, -1).map (c) -> parseInt(c, 16)
    ...
    expected = cols.split(/\s*÷\s*/).slice(0, -1).map (c) ->
      codes = c.split(/\s*×\s*/)
      codes.shift() if codes[0] is ''
    ...
    it cols, ->
      assert.deepEqual breaks, expected, i + ' ' + JSON.stringify(breaks) + ' != ' + JSON.stringify(expected) + ' #' + comment
